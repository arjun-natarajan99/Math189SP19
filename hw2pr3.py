...
def linreg(X, y, reg=0.0):
	...
	iden[0, 0] = 0 
	W_opt = np.linalg.solve(X.T @ X + reg * iden, X.T @ y)
	"*** END YOUR CODE HERE ***"
	return W_opt
...
def find_RMSE(W, X, y):
	...
	diff = y_predicted - y 
	MSE = (np.linalg.norm(diff) ** 2) / X.shape[0]
	RMSE = np.sqrt(MSE)
	"*** END YOUR CODE HERE ***"
	return RMSE
...
def RMSE_vs_lambda(X_train, y_train, X_val, y_val):
	...
	for i in range(len(reg_list)):
		RMSE_list.append(find_RMSE(W_list[i],X_val, y_val))
	"*** END YOUR CODE HERE ***"
	...
	"*** YOUR CODE HERE ***"
	reg_opt_index = np.argmin(RMSE_list)
	reg_opt = reg_list[reg_opt_index]
	"*** END YOUR CODE HERE ***"
	return reg_opt
...
def linreg_no_bias(X, y, reg=0.0):
	...
	rows = X.shape[0]
	ones = np.eye(rows)
	Aggregate = X.T @ (np.eye(rows) - np.ones(rows) / rows)
	W_opt = np.linalg.solve(Aggregate @ X + reg * np.eye(Aggregate.shape[0]), \
		Aggregate @ y)
	b_opt = sum((y - X @ W_opt)) / rows
	"*** END YOUR CODE HERE ***"
