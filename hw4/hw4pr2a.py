...
def NLL(X, y, W, reg=0.0):
	...
	"*** YOUR CODE HERE ***"
	sigma = sigmoid(X @ W)
	summand = np.multiply(y,np.log(sigma)) + np.multiply((1. - y),np.log(1.- sigma))
	nll = -sum(summand) + reg / 2 * np.linalg.norm(W) ** 2
...
def newton_step(X, y, W, reg=0.0):
	...
	"""
	"*** YOUR CODE HERE ***"
	logGrad = grad_logreg(X,y,W,reg)
	mu = sigmoid(X @ W)

	##consulted solutions for numpy syntax for diag and for regularization of H
	diag = np.diag( np.squeeze( np.asarray( np.multiply( mu, (1. - mu) ) ) ) )
	H = X.T @ diag @ X + reg * np.eye(X.shape[1])
	d = np.linalg.solve(H, logGrad)
	"*** END YOUR CODE HERE ***"
	return d
...
def get_description(X, y, W):
	...
	acc_count, prec_count, rec_count = 0, 0, 0
	prec_total, rec_total = 0, 0
	for i in range(m):
		actual = y.item(i)
	...
	accuracy = (1.0 * acc_count/m)
	precision = (1.0 * prec_count/prec_total)
	recall = (1.0 * rec_count/rec_total)
...
def plot_description(X_train, y_train, X_test, y_test):
	...
	"*** YOUR CODE HERE ***"
	#reg_list = [0.0, 0.01, 0.1, 0.3, 0.5, 0.75, 1.0, 5.0, 10.0, 20.0]
	reg_list = [0., 0.01, 0.1, 0.5, 1.0, 2.0, 5.0, 10.0, 20.0, 30.0]

	for i in range(len(reg_list)):
		W_opt, objs = grad_descent(X_train,y_train,reg_list[i],lr = 2e-4, print_freq = 100)
