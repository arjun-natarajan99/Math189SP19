...
def NLL(X, y, W, reg=0.0):
	...
	#consulted solutions for groundTruth and NLL
	mu = X @ W 
	exp_mu = np.exp(mu)
...
def accuracy_vs_lambda(X_train, y_train_OH, X_test, y_test, lambda_list):
	...
	for reg in lambda_list:
		...
		"*** YOUR CODE HERE ***"
		W_opt, nll_list = grad_descent(X_train,y_train_OH,reg, lr=2e-5,print_freq=50)
	...
	"*** YOUR CODE HERE ***"
	index = np.argmax(accu_list)
	reg_opt = lambda_list[index]
	"*** END YOUR CODE HERE ***"
